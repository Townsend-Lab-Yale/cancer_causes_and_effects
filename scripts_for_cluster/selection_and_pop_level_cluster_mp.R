...
print(paste0("MutPat version: ",packageVersion("MutationalPatterns")))
...
if(inputs["-tumor_name",] == "ESCA"){
  ...
  analysis <- cancereffectsizeR::trinuc_mutation_rates(cesa = analysis, 
                                                       signature_set = "COSMIC_v3.1", 
                                                       cores = as.numeric(inputs["-cores",]),
                                                       signatures_to_remove = escc_to_remove, sample_group = "ESCC")
...
save(analysis, file=paste0(run_note,"-MAF_for_analysis_after_mut_rate.RData"))

save_cesa(cesa = analysis, file=paste0(run_note,"-MAF_for_analysis_after_mut_rate.rds"))

analysis <-  ces_variant(cesa = analysis, 
                         cores=as.numeric(inputs["-cores",])) 

save(analysis,file = paste0(run_note,"-",paste(inputs["-tumor_name",],"_","_selection_output.RData",sep="")))
...
save(population_scaled_selection, file=paste0(run_note,"-",paste(inputs["-tumor_name",],"_","population_scaled_selection.RData",sep="")))
...
save(tumor_JSD_df, file = paste0(run_note,"-",paste(inputs["-tumor_name",],"_","tumor_JSD_df.RData",sep="")))
