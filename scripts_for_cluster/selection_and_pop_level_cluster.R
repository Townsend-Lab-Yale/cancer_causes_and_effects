...
library(tidyverse)
library(data.table)
library(cancereffectsizeR) # v.2.2.0

print(paste0("cancereffectsizeR version: ",packageVersion("cancereffectsizeR")))

ces_package <- packageVersion("cancereffectsizeR") %>% 
  as.character() %>%
...
if(!("-ready_MAF" %in% rownames(inputs))){
  ...
  MAF_for_analysis <- cancereffectsizeR::preload_maf(
    maf = inputs["-NCI_MAF",],
...
if(inputs["-tumor_name",] == "ESCA"){
  ...
  MAF_for_analysis$Unique_Patient_Identifier_short <- substr(MAF_for_analysis$Unique_Patient_Identifier,start = 1,stop = 12)
  for(tumor_ind in seq_along(unique(MAF_for_analysis$Unique_Patient_Identifier_short))){
    MAF_for_analysis[MAF_for_analysis$Unique_Patient_Identifier_short == unique(MAF_for_analysis$Unique_Patient_Identifier_short)[tumor_ind],"tumor_esca_type"] <- 
      esca_tumor_list[esca_tumor_list$CASE_ID == unique(MAF_for_analysis$Unique_Patient_Identifier_short)[tumor_ind],"ONCOTREE_CODE"]
  }
  ...
  analysis <- cancereffectsizeR::trinuc_mutation_rates(cesa = analysis, 
                                                       ...
                                                       signatures_to_remove = 
                                                         as.character(cancereffectsizeR::suggest_cosmic_signatures_to_remove(
                                                           cancer_type = "Eso-AdenoCA",treatment_naive = T
                                                         )), 
                                                       use_dS_exome2genome=TRUE,sample_group = "ESCA",signature_extractor = "deconstructSigs")
  
  # trinucs on ESCC
  # include signature 16 in accordance with https://www.annalsofoncology.org/article/S0923-7534(19)45461-6/fulltext
  
  escc_to_remove <- as.character(cancereffectsizeR::suggest_cosmic_signatures_to_remove(
  ...
  analysis <- cancereffectsizeR::trinuc_mutation_rates(cesa = analysis, 
                                                       ...
                                                       use_dS_exome2genome=TRUE,sample_group = "ESCC",signature_extractor = "deconstructSigs")
  ...
  to_remove = as.character(suggest_cosmic_signatures_to_remove(cancer_type = tumor_type_trinuc, treatment_naive = T))
  ...
  analysis <- cancereffectsizeR::trinuc_mutation_rates(analysis,signature_set = "COSMIC_v3.1", cores = as.numeric(inputs["-cores",]),
                                                       signatures_to_remove = to_remove, use_dS_exome2genome=TRUE,signature_extractor = "deconstructSigs")
...
save(analysis, file=paste0(ces_package,"-MAF_for_analysis_after_mut_rate.RData"))

save_cesa(cesa = analysis, file=paste0(ces_package,"-MAF_for_analysis_after_mut_rate.rds"))

analysis <-  ces_variant(cesa = analysis, 
                         cores=as.numeric(inputs["-cores",])) 

save(analysis,file = paste0(ces_package,"-",paste(inputs["-tumor_name",],"_","_selection_output.RData",sep="")))
...
save(population_scaled_selection, file=paste0(ces_package,"-",paste(inputs["-tumor_name",],"_","population_scaled_selection.RData",sep="")))
...
save(tumor_JSD_df, file = paste0(ces_package,"-",paste(inputs["-tumor_name",],"_","tumor_JSD_df.RData",sep="")))
