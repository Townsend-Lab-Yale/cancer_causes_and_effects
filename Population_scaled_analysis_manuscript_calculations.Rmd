...
these_colors["NFE2L2 R34P"] <- "#1b9e77"
these_colors["OR13G1 L79L"] <- "#e6ab02"
...
color_vec_fullname_fig1 <- setNames(object = c(color_vec_fullname["Tobacco (4,29)"],color_vec_fullname["Unknown, clock-like (5)"],color_vec_fullname["Non-actionable or unknown signatures"]),nm = c("Tobacco smoking (4)","Unknown, clock-like (5)","Unknown (8)"))
...
these_colors["Unknown (8)"] <- color_vec_fullname_fig1["Unknown (8)"]
these_colors["Unknown, clock-like (5)"] <- color_vec_fullname_fig1["Unknown, clock-like (5)"]
these_colors["Tobacco smoking (4)"] <- color_vec_fullname_fig1["Tobacco smoking (4)"]
...
#                                                  "Azathioprine treatment (used for immunosuppression) (32)",
#                                                  "Prior chemotherapy treatment (35)"),
#              `Mutagenic chemical exposure (22,24,42)` = c("Aristolochic acid exposure (22)",
#                                                           "Aflatoxin exposure (24)",
#                                                           "Occupational exposure to haloalkanes (42)")
#              
#            )
...
signatures_present_for_plot %>%
  ...
  mutate(Signature = 
           forcats::fct_recode(Signature,
                               `Deamination with age, clock-like (1)` = "Deamination with age (1)")) -> signatures_present_for_plot
...
action_effects <- as.data.frame(matrix(data = NA,nrow=length(unique(weights_combined_per_tumor_type$tumor_type)),
                                       ncol=length(actionable_signatures)+4),stringsAsFactors=F)
colnames(action_weights) <- c("tumor_type",clocklike_signatures,actionable_signatures,"Non-actionable or unknown signatures")
colnames(action_effects) <- c("tumor_type",clocklike_signatures,actionable_signatures,"Non-actionable or unknown signatures")
...
action_weights_m$Signature <- factor(action_weights_m$Signature,
                                     levels=c(clocklike_signatures,actionable_signatures,"Non-actionable or unknown signatures"),
                                     labels = c(signatures_names_matrix[c(clocklike_signatures,actionable_signatures),"Etiology_sig"],"Non-actionable or unknown signatures"))
...
action_effects_m %>% 
  filter(Signature == "Non-actionable or unknown signatures") %>% 
  arrange(desc(weight))
...
# Driver centric signature mutational effect size 


```{r load in data for driver centric figures}
# colors 
# color_vec_fullname
...
```{r function to generate figures}
...
driver_focused_nrsi_weight_barplot <- function(tumor_type_ourdata,
  ...
  driver_signature_nrsi_weight %>%
    ...
    summarize(summed_sig_nrsi = sum(summed_sig_nrsi)) %>%
    ungroup() -> 
  ...
  driver_signature_nrsi_weight_forplot <- driver_signature_nrsi_weight_full_sig %>%
    ...
    mutate(summed_sig_nrsi = case_when(signature_full %in% signatures_below_line_grouped ~ -summed_sig_nrsi, 
                                       TRUE ~ summed_sig_nrsi)) 
  
  # %>%
  #   mutate(Signature = gsub(pattern = "SBS",replacement = "",x = Signature)) %>%
  #   mutate(Signature = stringr::str_replace(string = Signature,pattern = "[:alpha:]",replacement = ""))
  # 
  # sigs_below_line_num <- gsub(pattern = "SBS",replacement = "",x = signatures_below_line)
  # sigs_below_line_num <- stringr::str_replace(string = sigs_below_line_num,pattern = "[:alpha:]",replacement = "")
  # sigs_below_line_num <- as.numeric(sigs_below_line_num)
  # 
  # 
  # 
  ...
  # if(below_line_black){
  #   
  #   sigs_here <- unique(as.numeric(driver_signature_nrsi_weight_forplot$Signature))
  #   sigs_here <- sort(sigs_here)
  #   sigs_here <- c(sigs_here[!sigs_here %in% sigs_below_line_num],sigs_below_line_num)
  #   
  #   sig_levels <- unique(as.character(sigs_here))
  #   color_code <- set_names(
  #     x = c(gg_color_hue(n = (length(sig_levels)-length(unique(sigs_below_line_num)))),rep(x = "black",length(unique((sigs_below_line_num))))),
  #     nm = sig_levels 
  #   )
  # }else{
  #   
  ...
  #   
  #   color_code <- set_names(
  #     x = gg_color_hue(n = length(sig_levels)),
  #     nm = sig_levels 
  #   ) 
  # }
  # 
  # driver_signature_nrsi_weight_forplot$Signature <-
  #   factor(driver_signature_nrsi_weight_forplot$Signature,levels = sig_levels)
  ...
  plot_range <- max(abs(driver_signature_nrsi_weight_forplot$summed_sig_nrsi))
  ...
  driver_plot_out <- ggplot(driver_signature_nrsi_weight_forplot) + 
    geom_bar(aes(x=variant_original,y = summed_sig_nrsi,fill=sig_char),stat="identity") + 
    scale_fill_manual(values = color_vec_fullname, na.value="black") +
    theme_bw() + 
  ...
  driver_plot_out <- ggplot(driver_signature_nrsi_weight_forplot) + 
    geom_bar(aes(y=fct_rev(variant_original),x = summed_sig_nrsi,fill=sig_char),stat="identity") + 
    scale_fill_manual(values = color_vec_fullname, na.value="black") +
    theme_bw() + 
    ...
    labs(
      title=paste(tumor_type_ourdata),
...
blca_output <- driver_focused_nrsi_weight_barplot(tumor_type_ourdata = "BLCA",
...
plot_grid(blca_output$avg_weight_plot + labs(title=NULL), blca_output$effect_plot,nrow = 1,rel_widths = c(1.1,5),align = "h")

plot_grid(blca_output$avg_weight_plot , blca_output$effect_plot + labs(title=NULL),nrow = 2,rel_heights =  c(1.4,5),align = "v")


ggsave(filename = "dev/BLCA_driver_focus_total_volume_sort.png",height = 4,width = 5)
```

```{r LUAD}

luad_output <- driver_focused_nrsi_weight_barplot(tumor_type_ourdata = "LUAD",
...
lusc_output <- driver_focused_nrsi_weight_barplot(tumor_type_ourdata = "LUSC",
...
plot_grid(lusc_output$avg_weight_plot + labs(title=NULL), lusc_output$effect_plot,nrow = 1,rel_widths = c(1,5),align = "h")


ggsave(filename = "dev/LUSC_driver_focus_total_volume_sort.png",height = 4,width = 5)
...
skcm_output <- driver_focused_nrsi_weight_barplot(tumor_type_ourdata = "SKCM_primary",
                                                  Bailey_tumor_type = "SKCM",
                                                  signatures_below_line = c("SBS7a","SBS7b","SBS7c","SBS7d"),
...
library(cowplot)
plot_grid(skcm_output$avg_weight_plot + labs(title=NULL), skcm_output$effect_plot,nrow = 1,rel_widths = c(1,5),align = "h")




ggsave(filename = "dev/SKCMp_driver_focus_total_volume_sort.png",height = 4,width = 5)
```
