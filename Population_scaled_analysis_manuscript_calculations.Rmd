...
library(tidyverse)
library(cancereffectsizeR) # use v2.1.2 
library(cowplot)
...
Here, we describe the method to calculate the population scaled selection intensity, using LUSC tumor `MDA-1229-T` as an example.

```{r load in figure 1 data}

tumor_pick <- "MDA-1229-T"
...
color_vec_fullname_fig1 <- setNames(object = c(color_vec_fullname["Tobacco (4,29)"],color_vec_fullname["Unknown, clock-like (5)"],color_vec_fullname["Non-actionable and unknown signatures"]),nm = c("Tobacco smoking (4)","Unknown, clock-like (5)","Unknown (8)"))
...
variant_levels <- c("TP53 R282W", "NFE2L2 R34P",  "OR13G1 L79L")
trinuc_levels <- c("C[C>T]G", "T[C>G]G", "C[C>A]A")
