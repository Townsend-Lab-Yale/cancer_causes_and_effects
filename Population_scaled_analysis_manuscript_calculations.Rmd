...
library(tidyverse)
library(cancereffectsizeR) # use v.0.1.1.9010 
# 'remotes::install_github("Townsend-Lab-Yale/cancereffectsizeR",ref = "140b261e5bd32ac064718d588e6b7310f3e601a4")'
library(cowplot)
...
sig_weights_all <- cancereffectsizeR::get_signature_weights(analysis,include_tumors_without_data = T)
...
sig_weights <- sig_weights_all %>% 
  ...
  mutate(name = signatures_names_matrix[name,2]) %>%
...
pop_scaled_selection_data <- per_tumor_data_main %>%
  ...
  mutate(Sig_name = signatures_names_matrix[Sig_name,2]) %>%
  group_by(variant) %>%
...
rownames(signatures_cosmic_May2019) <- signatures_names_matrix[rownames(signatures_cosmic_May2019),2]

signatures_cosmic_May2019 <- signatures_cosmic_May2019[rownames(signatures_cosmic_May2019) %in% sig_weights$name,]

signatures_m <- signatures_cosmic_May2019 %>% 
...
contributed_flux <- population_scaled_selection %>% 
  ...
  mutate(signatures = signatures_names_matrix[as.character(signatures),2])
...
```{r effect size plot}
effect_size_data <- analysis$selection %>% 
  mutate(variant = variant_name_extracter(variant)) %>% 
...
top_weights <- weights_combined %>% 
  ...
  mutate(signature_name = signatures_names_matrix[Signature,2])

sigs_present_ordered <- signatures_names_matrix[signatures_names_matrix[,1] %in% as.vector(as.matrix(signatures_present)),2]
...
for(tumor_ind in 1:length(tumors_in_4x4_plot)){
  ...
  these_data_nozero_forplot <- these_data_nozero %>%
    mutate(Signature = signatures_names_matrix[as.character(Signature),2]) %>%
...
per_tumor_combined$Signature_long <- signatures_names_matrix[per_tumor_combined$Signature,2]
per_tumor_combined$Signature <- factor(per_tumor_combined$Signature, levels=signatures_names_matrix[,1])
per_tumor_combined$Signature_long <- factor(per_tumor_combined$Signature_long, levels=signatures_names_matrix[,2])
...
weights_combined_per_tumor_type$Signature_long <- signatures_names_matrix[weights_combined_per_tumor_type$Signature,2]
...
weights_combined_per_tumor_type$Signature_long <- factor(weights_combined_per_tumor_type$Signature_long, levels=signatures_names_matrix[,2])
...
signatures_names_matrix["SBS32",2] <- "Treatment with azathioprine (32)"
signatures_names_matrix["SBS31",2] <- "Prior treatment with platinum drugs (31)"
signatures_names_matrix["SBS35",2] <- "Prior treatment with platinum drugs (35)"
signatures_names_matrix["SBS38",2] <- "Potentially indirect damage from UV light (38)"
signatures_names_matrix["SBS42",2] <- "Exposure to haloalkanes (42)"
signatures_names_matrix["SBS1",2] <- "Deamination with age, clock-like (1)"
...
action_weights_m$Signature <- factor(action_weights_m$Signature,
                                     ...
                                     labels = c(signatures_names_matrix[c(clocklike_signatures,actionable_signatures),2],"Unknown signatures"))
