...
library(tidyverse)
library(cancereffectsizeR) # used v2.2.0 

#J. Mandell fix for not having to be online to load in ces.refset.hg19 package
...
# set the colors for the manuscript figures
...
load("input_data/from_cluster/MP_2_2_0-LUSC__selection_output.RData")
load("input_data/from_cluster/MP_2_2_0-combined_per_tumor_proportion_scaled_selection.RData")
load("input_data/from_cluster/MP_2_2_0-LUSC_population_scaled_selection.RData")
load("input_data/from_cluster/MP_2_2_0-JSD_combined.RData")
load("input_data/from_cluster/MP_2_2_0-weights_combined.RData")
load("input_data/from_cluster/MP_2_2_0-combined_substitution_data.RData")

# Some of the TCGA tumors in the "ESCA" dataset are actually ESCC
per_tumor_data_main <- per_tumor_data_main %>%
  mutate(tumor_type = case_when(
    tumor_type == "ESCA" ~ "ESCA and ESCC", 
    TRUE ~ tumor_type))

JSD_combined <- JSD_combined %>%
  mutate(tumor_type = case_when(
    tumor_type == "ESCA" ~ "ESCA and ESCC", 
    TRUE ~ tumor_type))
...
names(total_substitutions)[which(names(total_substitutions)=="ESCA")] <- "ESCA and ESCC"
...
Here, we describe the method to calculate the population scaled selection intensity, using LUSC tumor `TCGA-98-A53J-01A-11D-A26M-08` as an example.

```{r load in figure 1 data}

tumor_pick <- "TCGA-98-A53J-01A-11D-A26M-08"
...
these_colors["KLF5 E419Q"] <- "#1b9e77"
these_colors["OR2T34 L163L"] <- "#e6ab02"
these_colors["TP53 R282W"] <- "#d95f02"
these_colors[as.character(sig_weights$name)] <- "placeholder"


sig_weights %>%
  mutate(name = factor(name,levels = signatures_names_matrix[,"Etiology_sig"])) %>%
  mutate(name = 
  ...
  group_by(name) %>%
  summarize(value = sum(value)) -> sig_weights_named



color_vec_fullname_fig1 <- setNames(object = 
                                      c(color_vec_fullname["Deamination with age, clock-like (1)"],
                                        color_vec_fullname["APOBEC (2,13)"],
                                        color_vec_fullname["Unknown, clock-like (5)"],
                                        color_vec_fullname["Non-actionable and unknown signatures"]),
                                    nm = c("Deamination with age (1)","APOBEC (2)","Unknown, clock-like (5)","Damage by reactive oxygen species (18)"))

# color_vec_fullname_8 <- c(color_vec_fullname, setNames(object = "black",nm = "Unknown (8)"))

sig_weights_named$name <- forcats::fct_reorder(sig_weights_named$name,sig_weights_named$value)
...
signature_proportion_plot <- 
  ...
  scale_fill_manual(values = color_vec_fullname_fig1,
                    name="Signature") +
...
pop_scaled_selection_data$variant 
...
variants = select_variants(cesa = analysis, variant_passlist = my_aac_muts, include_subvariants = T)
...
```{r}
# my_aac_muts = c("KLF5_E419Q", "OR2T34_L163L", "TP53_R282W")
signatures_m$variant[which(signatures_m$name == "G[C>T]A")] <- "OR2T34 L163L"
signatures_m$variant[which(signatures_m$name == "T[C>G]A")] <- "KLF5 E419Q"
signatures_m$variant[which(signatures_m$name == "C[C>T]G")] <- "TP53 R282W"

variant_levels <- c("TP53 R282W", "KLF5 E419Q",  "OR2T34 L163L")
trinuc_levels <- c("C[C>T]G", "T[C>G]A", "G[C>T]A")

levels(sig_weights_named$name)

# signature_levels <- c("Unknown (8)", "Tobacco smoking (4)", "Unknown, clock-like (5)")

signature_levels <- sig_weights$name
...
signatures_m_relevant$cols_to_plot="bjslack"
...
signature_trinuc_weights_ordered <- ggplot(data = signatures_m_relevant, 
                                           aes(x=name,y=value)) + 
  geom_bar(stat="identity", aes(color=variant,
                                fill=Signatures)) + 
  facet_wrap(facets = "Signatures",ncol = 1,scales = "free_y") + 
  scale_fill_manual(values = color_vec_fullname_fig1,na.value="gray") + 
...
these_colors["Deamination with age (1)"] <- color_vec_fullname_fig1["Deamination with age (1)"]
these_colors["Unknown, clock-like (5)"] <- color_vec_fullname_fig1["Unknown, clock-like (5)"]
these_colors["APOBEC (2)"] <- color_vec_fullname_fig1["APOBEC (2)"]
these_colors["Damage by reactive oxygen species (18)"] <- color_vec_fullname_fig1["Damage by reactive oxygen species (18)"]
...
contributed_flux <- population_scaled_selection %>% 
  ...
  group_by(variant) %>%
  # normalize contributed weight to sum to 1 
  mutate(value = value / sum(value)) %>% 
...
prop_mut_source_effect_size <- ggplot(data = this_pop_scaled_effect_size, aes(x=variant, y= pct)) + 
  ...
  theme_classic() + 
  theme(legend.position = "none") + 
  labs(y="Proportional mutation source effect size", x="Variant")+
...
## JSD, exploring and visualizing the data
...
signatures_to_color <- 15
tumors_in_4x4_plot <- c("SKCM_primary","COAD","HNSC_HPVneg","THCA")
...
for(tumor_ind in 1:length(tumors_in_4x4_plot)){
  ...
  if(this_tumor_type == "OV") { 
    ...
    these_JSD[5,] <- these_JSD_full[these_JSD_full$Tumor=="TCGA-24-1103-01A-01W-0488-09",]
  }
...
these_colors <- setNames(nm = sigs_present_ordered,rep("red",length(sigs_present_ordered)))
...
these_colors[c("APOBEC (2)","APOBEC (13)")] <- color_vec_fullname["APOBEC (2,13)"]
these_colors["Deamination with age (1)"] <- color_vec_fullname["Deamination with age, clock-like (1)"]
these_colors["Defective homologous recombination (3)"] <- color_vec_fullname["Defective homologous recombination (3)"]
these_colors["Tobacco smoking (4)"] <- color_vec_fullname["Tobacco (4,29)"]
these_colors["Unknown, clock-like (5)"] <- color_vec_fullname["Unknown, clock-like (5)"]
these_colors[c("UV light (7b)","UV light (7c)","UV light (7d)")] <- color_vec_fullname["UV light (7a–d,38)"]
these_colors[c("Polymerase eta somatic hypermutation (9)","Unknown (39)","Unknown (40)")] <- "black"


signatures_present_for_plot %>%
  mutate(Signature = 
           forcats::fct_collapse(
             Signature,
             `UV light (7a–d,38)` = c("UV light (7a)",
                                      ...
                                      "UV light (7a,7b,7c,7d)"),
             `APOBEC (2,13)` = c("APOBEC (2)","APOBEC (13)"),
             `Tobacco (4,29)` = c("Tobacco smoking (4)", "Tobacco chewing (29)"),
             `Prior treatment (11,31,32,35)` = c("Temozolomide treatment (11)",
                                                 "Prior treatment with platinum drugs (31)",
                                                 "Treatment with azathioprine (32)",
                                                 "Prior treatment with platinum drugs (35)"),
             `Mutagenic chemical exposure (22,24,42,88)` = c("Aristolochic acid exposure (22)",
                                                             "Aflatoxin exposure (24)",
                                                             "Occupational exposure to haloalkanes (42)",
                                                             "Colibactin exposure (COSMIC 3.1) (88)")
  ...
  mutate(Signature = 
           forcats::fct_recode(Signature,
                               `Deamination with age, clock-like (1)` = "Deamination with age (1)",
                               `Alcohol-associated (16)` = "Unknown (16)")) -> signatures_present_for_plot


signatures_present_for_plot


legend_for_plot <-  suppressWarnings(  cowplot::get_legend( ggplot(signatures_present_for_plot, aes(fill=Signature)) + 
                                                              geom_bar(aes(x=xx,y=yy),stat = "identity",color="black") + 
                                                              scale_fill_manual(values = c(color_vec_fullname[names(color_vec_fullname) %in% signatures_present_for_plot$Signature],setNames(object = "black",nm = "Other signatures")),na.value="black") + 
...
for(tumor_ind in 1:length(tumors_in_4x4_plot)){
  ...
  if(this_tumor_type == "SKCM_primary"){
    these_NRSI <- per_tumor_data_main[per_tumor_data_main$tumor_type=="SKCMP",]
  }else{
  ...
  these_data_nozero$data_type <- factor(these_data_nozero$data_type, levels=c("signature_weights","NRSI"),labels = c("SW", "NRSI weights"))
  ...
  these_data_nozero_forplot %>%
    mutate(sig_collapsed = 
             forcats::fct_collapse(
               Signature,
               `UV light (7a–d,38)` = c("UV light (7a)",
                                        "UV light (7b)",
    ...
    mutate(sig_collapsed = 
             forcats::fct_recode(sig_collapsed,
                                 `Deamination with age, clock-like (1)` = "Deamination with age (1)",
                                 `Alcohol-associated (16)` = "Unknown (16)")) %>% 
    group_by(Tumor,data_type,bar_label_2,sig_collapsed) %>%
    summarize(Weight_prop = sum(Weight_prop)) -> these_data_nozero_forplot
  
  
  these_data_nozero_forplot <- these_data_nozero_forplot %>%
    mutate(data_type = forcats::fct_recode(data_type, 
                                           `CEW` = "NRSI weights"))
  
  these_data_nozero_forplot <- these_data_nozero_forplot %>% 
    mutate(tumor_factor_lvl = as.numeric(Tumor)) %>% 
    mutate(Tumor_short = str_sub(Tumor, start = 1,end = 12)) %>%
    arrange((tumor_factor_lvl)) %>%
    mutate(Tumor_short = fct_inorder(Tumor_short))
  ...
  plot_title <- this_tumor_type
  if(this_tumor_type=="HNSC_HPVneg"){
    plot_title <- "HPV-negative HNSC"
  }
  if(this_tumor_type == "SKCM_primary"){
    plot_title <- "Primary SKCM"
  }
  ...
  JSD_plot_ggplot <- ggplot(these_JSD_full, aes(y=JSD, x=1,label=Tumor)) + 
    ...
    labs(y="Jensen-Shannon Divergence", title=plot_title)+ 
...
weight_and_effect_data %>%
  filter(Signature == "SBS1" & tumor_type == "LGG")
...
```{r actionable signatures data analysis}
...
actionable_signatures <- c("SBS2",
                           ...
                           "SBS11","SBS22","SBS24",
                           "SBS16",
                           "SBS29","SBS31","SBS32","SBS35","SBS38","SBS42","SBS88")
...
action_effects <- as.data.frame(matrix(data = NA,nrow=length(unique(weights_combined_per_tumor_type$tumor_type)),
                                       ncol=length(actionable_signatures)+4),stringsAsFactors=F)
colnames(action_weights) <- c("tumor_type",clocklike_signatures,actionable_signatures,"Non-actionable and unknown signatures")
colnames(action_effects) <- c("tumor_type",clocklike_signatures,actionable_signatures,"Non-actionable and unknown signatures")
...
action_weights_m <- pivot_longer(data = action_weights, cols = -tumor_type, names_to = "Signature",values_to = "weight")
action_effects_m <- pivot_longer(data = action_effects, cols = -tumor_type, names_to = "Signature",values_to = "weight")
...
action_weights_m$Signature <- factor(action_weights_m$Signature,
                                     levels=c(clocklike_signatures,actionable_signatures,"Non-actionable and unknown signatures"),
                                     labels = c(signatures_names_matrix[c(clocklike_signatures,actionable_signatures),"Etiology_sig"],"Non-actionable and unknown signatures"))
...
action_weights_m %>%
  ...
  mutate(Signature = 
           forcats::fct_recode(Signature,
                               ...
                               `Alcohol-associated (16)` = "Unknown (16)")) %>% 
  group_by(tumor_type,Signature) %>%
  summarize(weight = sum(weight)) -> action_weights_m_collapsed


# sideways

action_weights_m_plot <- ggplot(action_weights_m_collapsed) + 
...
action_effects_m %>%
  ...
  mutate(Signature = 
           forcats::fct_recode(Signature,
                               ...
                               `Alcohol-associated (16)` = "Unknown (16)")) %>% 
  group_by(tumor_type,Signature) %>%
  summarize(weight = sum(weight)) -> action_effects_m_collapsed




action_effects_m_plot <- ggplot(action_effects_m_collapsed) + 
  geom_bar(aes(x=tumor_type,y=weight,fill=Signature),stat="identity") + 
  theme_bw() + 
  # theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) +
  scale_fill_manual(values = color_vec_fullname) + 
...
weight_effect_legend <- suppressWarnings( cowplot::get_legend(action_effects_m_plot))
...
action_effects_m %>% 
  ...
  group_by(tumor_type) %>%
  summarize(total_aging_effect_weight = sum(weight)) %>%
  arrange(total_aging_effect_weight) %>% 
  knitr::kable()
...
action_effects_m %>% 
  filter(Signature == "Non-actionable and unknown signatures") %>% 
  arrange(desc(weight))

weights_combined_per_tumor_type_unknown <- weights_combined_per_tumor_type[grep(pattern = "Unknown",x = weights_combined_per_tumor_type$Signature_long),]

# top tumor types of unknown summed effect
weights_combined_per_tumor_type_unknown %>%
  filter(!Signature_long == "Unknown, clock-like (5)") %>%
  filter(!Signature_long == "Unknown (16)") %>%
  group_by(tumor_type) %>% 
  summarize(total_unknown = sum(summed_weight_prop)) %>%
  arrange(desc(total_unknown))



action_effects_m %>%
  filter(Signature == "Deamination with age, clock-like (1)") %>%
  arrange(weight)
...
cowplot::save_plot(filename = "output_data/figure_4_weight_effect.png",plot = combined_clocklike_w_dotplot,
...
wilcoxon_results_binned %>%
  filter(tumor_type == "LUAD", 
         Signature %in% c("Tobacco smoking (4)","Tobacco chewing (29)") ) 
...
# Driver centric signature mutational effect size, Figure 3 


```{r driver_centric_figure}
source("dev/driver_plots.R")
```
